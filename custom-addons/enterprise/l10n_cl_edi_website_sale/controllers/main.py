...
class L10nCLWebsiteSale(WebsiteSale):
    ...
    def l10n_cl_invoicing_info(self, **kw):
        ...
        if (values['errors_fields'] or values['errors_empty']) and kw['l10n_cl_type_document'] == 'ticket':
            order_sudo.partner_invoice_id = order_sudo.partner_shipping_id.id
            order_sudo.partner_shipping_id.write({'l10n_latam_identification_type_id' : 4})
            order_sudo.partner_id.write({'l10n_cl_sii_taxpayer_type' : '3'})
            order_sudo.partner_id.write({'l10n_cl_activity_description' : "PARTICULAR"})
        else:
            order_sudo.partner_id.write({'company_type' : 'company'})
            order_sudo.partner_invoice_id = order_sudo.partner_shipping_id.id
            order_sudo.partner_shipping_id.write({'l10n_latam_identification_type_id' : 4})
            if 'l10n_cl_dte_name' in kw:
                order_sudo.partner_invoice_id.name = kw['l10n_cl_dte_name'] 
